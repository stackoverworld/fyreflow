...
- If provider status still shows `CLI missing`, verify binaries on backend (`which claude`, `which codex`) and set `CODEX_CLI_PATH` / `CLAUDE_CLI_PATH` when binaries are outside `PATH`.
