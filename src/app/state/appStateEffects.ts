...
const GENERIC_CANCELLATION_PATTERN =
  /\b(cancelled|canceled|aborted|operation was aborted|stopped by user|run stopped)\b/i;
const HIGH_SIGNAL_FAILURE_PATTERN =
  /\b(token[_ ]?expired|401|403|unauthorized|forbidden|could not resolve host|enotfound|eai_again|getaddrinfo|dns|malformed|invalid url|quality gates?|step output must be valid json|timed out)\b/i;
...
function isGenericCancellationSignal(value: string | undefined): boolean {
  if (!value || value.trim().length === 0) {
    return false;
  }
  return GENERIC_CANCELLATION_PATTERN.test(value);
}

function pickPreferredFailureCandidate(candidates: string[]): string | undefined {
  const normalizedCandidates = candidates
    .map((candidate) => normalizePreview(stripRunFailedPrefix(candidate), RUN_FAILURE_DETAIL_PREVIEW_LENGTH))
    .filter((candidate): candidate is string => Boolean(candidate));
  if (normalizedCandidates.length === 0) {
    return undefined;
  }

  const highSignal = normalizedCandidates.find(
    (candidate) => HIGH_SIGNAL_FAILURE_PATTERN.test(candidate) && !isGenericCancellationSignal(candidate)
  );
  if (highSignal) {
    return highSignal;
  }

  const nonGeneric = normalizedCandidates.find((candidate) => !isGenericCancellationSignal(candidate));
  if (nonGeneric) {
    return nonGeneric;
  }
...
function selectPrimaryFailedStep(run: PipelineRun): PipelineRun["steps"][number] | undefined {
  const failedSteps = run.steps.filter((step) => step.status === "failed");
  if (failedSteps.length === 0) {
    return undefined;
  }

  const informative = [...failedSteps].reverse().find((step) => {
    const signal = `${step.error ?? ""}\n${step.output ?? ""}`.trim();
    if (signal.length === 0) {
      return false;
    }
    return !isGenericCancellationSignal(signal);
  });
...
function extractRunFailureReason(run: PipelineRun, failedStep: PipelineRun["steps"][number] | undefined): string | undefined {
  ...
  const preferred = pickPreferredFailureCandidate(candidates);
  return preferred ? normalizePreview(preferred, RUN_FAILURE_REASON_PREVIEW_LENGTH) : undefined;
}
...
export function buildRunCompletionModalContext(run: PipelineRun): RunCompletionModalContext {
  ...
  if (status === "failed") {
    const failedStep = selectPrimaryFailedStep(run);
...
export function buildRunInputFallbackSummary(context: RunInputModalContext): string {
  if (context.requests.length === 0 && context.blockers.length > 0) {
    return context.blockers[0]?.message || "Run is blocked and needs attention.";
  }
...
export function buildRunInputModalSignature(context: RunInputModalContext): string {
  return `${context.source}:${context.pipelineId}:${context.runId ?? "none"}:${context.requests
    ...
    .join(",")}:${context.blockers.map((blocker) => blocker.id).join(",")}:${context.summary}`;
}
